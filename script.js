    ...
    async function initializeApp(user) {
        ...
        async function updateDashboard() {
            ...
            clinics.forEach(clinic => {
                const stageId = stages.findIndex(s => s.name === clinic.stage);
                const stageClass = stageId > -1 ? stages[stageId].id : 'awareness';
        ...
        async function renderTodoList() {
            ...
            const allTodos = await todosCollection.get().then(snap => snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        ...
        async function showDetailView(id) {
            ...
            const detailClinicName = document.getElementById('detail-clinic-name');
            if(detailClinicName) detailClinicName.textContent = clinic.name;
            // ... (find and set all other detail elements)
        ...
        function loadMap(address, name) {
            ...
            // Naver Map Script가 이미 Head에 로드되어 있다고 가정하고 바로 사용
        ...
        document.getElementById('add-clinic-btn').addEventListener('click', () => { clinicForm.reset(); document.getElementById('modal-title').textContent = '의원 정보 입력'; document.getElementById('clinic-id').value = ''; modal.classList.remove('hidden'); });
        document.getElementById('edit-clinic-btn').addEventListener('click', async () => { if (!currentClinicId) return; const doc = await clinicsCollection.doc(currentClinicId).get(); if (doc.exists) { const clinicToEdit = { id: doc.id, ...doc.data() }; document.getElementById('modal-title').textContent = '의원 정보 수정'; document.getElementById('clinic-id').value = clinicToEdit.id; document.getElementById('clinic-name').value = clinicToEdit.name; const addressParts = clinicToEdit.address.split(','); document.getElementById('clinic-address').value = addressParts[0].trim(); document.getElementById('clinic-address-detail').value = addressParts.length > 1 ? addressParts.slice(1).join(',').trim() : ''; document.getElementById('clinic-manager').value = clinicToEdit.manager; document.getElementById('clinic-contact').value = clinicToEdit.contact; document.getElementById('clinic-department').value = clinicToEdit.department; document.getElementById('clinic-scale').value = clinicToEdit.scale; document.getElementById('clinic-notes').value = clinicToEdit.notes; document.getElementById('clinic-stage').value = clinicToEdit.stage; modal.classList.remove('hidden'); } });
