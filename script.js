document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initializeApp(user) {
        ...
        // 사용자 이메일 표시 및 로그아웃 버튼 이벤트
        if(userEmailSpan) userEmailSpan.textContent = user.email;
        if(logoutBtn) logoutBtn.addEventListener('click', () => auth.signOut());

        // --- 데이터 관리 함수 ---
        const getClinics = async () => (await clinicsCollection.orderBy('updatedAt', 'desc').get()).docs.map(doc => ({ id: doc.id, ...doc.data() }));
        const getTodos = async () => (await todosCollection.get()).docs.map(doc => ({ id: doc.id, ...doc.data() }));
        ...
        function loadMap(address, name) {
            ...
            else { const mapScript = document.createElement('script'); mapScript.type = 'text/javascript'; mapScript.src = `https://oapi.map.naver.com/openapi/v3/maps.js?ncpKeyId=d7528qc21z&submodules=geocoder`; mapScript.onload = drawMap; document.head.appendChild(mapScript); }
        }
