...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 로그인 버튼 리스너를 바깥으로 빼서 한 번만 등록되도록 함
    if (loginBtn) {
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            ...
            appInitialized = false; // 로그아웃 시 다시 초기화 가능하도록 설정
    ...
    async function initializeApp(user) {
        ...
        addClinicBtn.addEventListener('click', () => { clinicForm.reset(); modalTitle.textContent = '의원 정보 입력'; clinicModal.classList.remove('hidden'); });
        ...
        historyBtn.addEventListener('click', () => { buildHistoryHtml(); historyModal.classList.remove('hidden'); });
