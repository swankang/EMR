    ...
    async function initializeApp(user) {
        ...
        function loadNaverMapsApi() {
            return new Promise((resolve, reject) => {
                ...
                mapScript.src = `https://oapi.map.naver.com/openapi/v3/maps.js?ncpKeyId=YOUR_NAVER_MAPS_CLIENT_ID&submodules=geocoder`;
        ...
        document.querySelector('#clinic-modal .close-btn').addEventListener('click', () => {
            document.getElementById('clinic-modal').classList.add('hidden');
        });
        document.getElementById('clinic-modal').addEventListener('click', (e) => {
            if (e.target.id === 'clinic-modal') e.target.classList.add('hidden');
        });
        searchAddressBtn.addEventListener('click', execDaumPostcode);
